...
void main()    {
	...
	H = normalize(L + E);
	// (*) We don't pass a 'naked' mMatrix in shaders (not robust to double precision usage). We dress it in a mvMatrix.
	// So here we're passing a mMatrix from camera space to light space (through a mvMatrix).
